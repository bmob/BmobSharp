...
//        });
	...
	void findAllUser ()
	{
		...
		void FileUpload(){		
			...
			//	Byte[] data = null;
			//	 using (var stream = File.OpenRead("/Users/BmobZq/Desktop/b.png"))
			//	 {
			//  		 data = stream.ReadAsBytes();
			//	 } 
			//	//其中"abc.png"是文件名，一定要有后缀
			//	Bmob.FileUpload(new BmobLocalFile(data, "abc.png"), (resp, exception) => {
			//			if (exception != null) {
			...
			// 第一个参数是文件路径
			Bmob.FileUpload("/Users/BmobZq/Desktop/b.png", (resp, exception) => {
					if (exception != null) {
						print ("上传请求失败, 失败原因为： " + exception.Message);
						return;
					}
		...
		void FileDelete(){
			// 第一个参数是 cdn+"/"+URL
			// 其中cdn是指上传文件后再body返回的cdn,其中URL指的是上传文件后在body中返回的url除去域名之后的字符串。
	    	Bmob.FileDelete("upyun" + "/" + "2017/05/03/34440dd640bf19eb801decfeac83911e.png", (resp, exception) => {
					if (exception != null) {
						print ("删除文件请求失败, 失败原因为： " + exception.Message);
						return;
					}
					
					print ("删除文件请求发送成功！" + toString (resp));
			});
		...
		void OnGUI ()
		{
				...
				btnTop += btnHeight + 10 * scale;
				if (GUI.Button (new Rect ((Screen.width - btnWidth) / 2 - btnWidth, btnTop, btnWidth, btnHeight), "FileDelete")) {
						FileDelete ();
				}
